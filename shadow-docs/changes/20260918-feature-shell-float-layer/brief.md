---
{
  ...
  "workflow": {
    "operation": null,
    "checkpoint": null,
    "planHash": "8d46a1b37dd608c031e213204fec1ff66c623984b2c777846f30ce5ff6e7ebb9",
    "updatedAt": null,
    "lastError": null
  }
}
...
## 动机
「一切皆插件」架构愿景的第一步。当前预览区是内核硬编码的右侧固定分栏（`App.tsx` work-area 中 `preview-area` 恒占 50% 宽），不用时白白浪费一半空间，且"预览"作为布局常量存在——用户需要的是**按需唤起**。本变更把预览改为应用内浮窗：内核提供一个通用浮窗层 slot（可拖拽/缩放/最小化/多开），manifest `views.area` 从 `preview` 泛化为 `float`，任何插件视图都能以浮窗形态打开。这是后续 editors 贡献点（变更 2）与内置官方插件化（变更 3）之前必须先落地的内核能力。

## 引用规范
- shadow-docs/knowledge/renderer-shell-routing.md
  - 当前结论: mainView 双层路由；work 渲染 ActivityBar+侧栏+work-area；全屏视图卸载 work-area 但 activePanel 状态保持不丢
  - 适用 scope: src/renderer/src
  - 遵循: 浮窗层挂在 work 视图内，不动 mainView 机制；`mainView === 'settings'` 时浮窗随 work 卸载，开合状态与几何由 floats 注册表保持，返回后还原（与 activePanel 同语义）
...
- **理由:** 「视图=内容，区域=slot」与插件驱动模型同构；进程内浮窗主题/状态零同步成本。Knowledge 遵循：声明制先改契约，快照模式复用，token/动效/a11y 按既有规范。
- **前置依赖:** 20260917-feature-shell-chrome-plugin-api（PR #2）先合入 main——本变更基于其图标注册表与 statusItems 快照模式开分支。
- **非目标:** 主区 tabs（接口留编辑器组抽象）、标题栏贡献点、SDK `wuh.ui` float 运行时 API（随变更 2）、浮窗几何持久化（本轮内存态）、弹出 OS 窗口。

## 任务
### Phase 1 贡献点契约（声明制先行）
- [ ] `ViewArea` 改为 `'sidebar' | 'float'`，`validateManifest` 校验与错误文案同步更新 — `src/shared/plugin.ts` — 修改
- [ ] manifest 测试用例更新：float 合法 / preview 拒绝 / 多 float 视图共存 — `tests/plugin-manifest.test.ts` — 修改
- [ ] preview-markdown 插件迁移：`area: "preview"` → `"float"` — `plugins/preview-markdown/plugin.json` — 修改

### Phase 2 内核浮窗层
- [ ] floats 注册表（新增）：open/close/toggle/minimize/geometry 状态，useSyncExternalStore 快照模式，`commit()` 产新引用 — `src/renderer/src/plugins/floats.ts` — 新增
- [ ] FloatLayer 组件（新增）：头部拖拽、边缘/角缩放、最小化角落 chip、z 序与焦点管理、Esc 关闭聚焦浮窗、token 化动效 + reduced-motion — `src/renderer/src/components/FloatLayer.tsx` `src/renderer/src/styles/global.css` — 新增
- [ ] 帧宿主适配：`listPreviewViews` → `listFloatViews`，浮窗内复用 `PluginView` 帧宿主 — `src/renderer/src/plugins/PluginFrameHost.tsx` — 修改

### Phase 3 壳层整合与验证
- [ ] App.tsx：work-area 移除 preview-area 列（editor-area 占满）；float 视图注入 ActivityBar toggle 项；mainView 切换时浮窗卸载/还原 — `src/renderer/src/App.tsx` — 修改
- [ ] ActivityBar 支持 toggle 型 item（激活指示=浮窗打开，区别于面板选中） — `src/renderer/src/components/ActivityBar.tsx` `src/renderer/src/styles/global.css` — 修改
- [ ] 回归：`pnpm typecheck` + `pnpm test` — 仓库根 — 验证
- [ ] 手动走查：四主题×亮暗下浮窗拖拽/缩放/最小化/Esc/多开，设置页进出后浮窗还原，reduced-motion — 手动 — 验证
