...
- `vitest run tests/editor-codemirror.test.ts tests/home-editor-panel.test.ts tests/editor-live-preview-toggle.test.ts tests/editor-page.test.tsx tests/doc-transfer.test.ts`（CM6 适配/双通道语义/命令契约/大纲/字数/即时渲染开关时序纯逻辑 + `/editor` 顶栏与冷启动草稿会话 + 面包屑完整路径与改名/迁移命令发布 + docTransfer 纯逻辑）。
