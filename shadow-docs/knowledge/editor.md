...
domain: renderer-ui
keywords: [主编辑器, MarkdownEditor, CodeMirror, CM6, 预览, PreviewPane, renderPipeline, 命令通道, editor-commands, 双通道, 防回环, 图片粘贴, 大纲, 字数, 主题桥接, HighlightStyle]
...
- `vitest run tests/editor-codemirror.test.ts tests/home-editor-panel.test.ts`（CM6 适配/双通道语义/命令契约/大纲/字数纯逻辑）。
