...
**SideMenu（2026-09-20 起，取代 ActivityBar）**：左栏菜单栏，两形态**瞬时切换**（禁 width 过渡，ui-patterns 布局位移动画禁令）——收起 = 48px 图标 rail，展开 = 220px 图标+文字（label 仅 opacity 淡入 200ms）。能力沿袭：左缘 2px 激活指示条（`::before`）、徽标位（数字 99+ 折叠 / `dot` 圆点，展开态转行内）、`data-tip` 自绘 tooltip（**仅收起态**显示，hover 与 `:focus-visible` 可见）+ `aria-label`、toggle 型 item（`toggleItems`/`openToggleKeys`/`onToggle`，激活态=浮窗打开 `aria-pressed`，区别于菜单选中态 `aria-current="page"`）、导航子树（展开态挂载 `TreeWrap`）。结构：顶部**直接是导航项** → toggle 组（浮窗开关）→ **底部系统区两项制**（`margin-top:auto` 吸附左下，20260924-feature-sidemenu-settings-item 取代 20260924-feature-sidemenu-bottom-toggle 的两行制）：**【用户】在上（点击 /account、hover 快捷面板）+【设置】在下**（`IconSettings`；展开态点击直达 `/settings` 且 `settingsActive` 高亮 `aria-current="page"`，**收起态点击仅 `onToggleExpanded` 展开菜单不导航**，data-tip 复合「设置 · 展开菜单」）——收起态专属展开钮已退役（展开职责由设置项承担），⌘/Ctrl+B 全局不变。低频 chrome 控件不占导航黄金位。

**用户入口与快捷面板（20260924-feature-sidemenu-settings-item 瘦身化；二级 popover 化 + 用户中心直达自 2026-09-22）**：底部两项制的用户项 = **图标恒为品牌标 IconLogo（头像占位）**——GitHub 头像投影（UserAvatar/PopAvatar img）当日即回退：远程图片（avatars.githubusercontent.com）网络不可靠时常破损，**壳层禁止渲染远程头像图片，头像显示待 Settings「用户设置」（本地 name/avatar）接管**；展开态名字保留 GitHub 用户名文本投影（`name‖login`，未授权/stale 回落 wuh-site，文本走 API 响应无网络依赖，数据源见 renderer-shell-routing 卡「全局身份 store」段）；版本行随展开态显示；**点击**进入用户中心 `/account`（GitHub 授权 + 身份/仓库 + Git 提交身份，`userActive` 命中 `/account` 时高亮），**悬停/键盘聚焦**弹出 `UserQuickPanel`（`role="menu"`，aria-label「外观与语言」= `pop.quickAria`）：**主题（酒红/素雅）、外观（跟随系统/浅色/深色）、语言（中文/English/日本語）三组二级 popover 行**——通用 `PopSubmenu` 组件：行左文案右 chevron、hover/聚焦弹出选项子菜单（`menuitemradio`、当前项勾选、180ms 延迟移入、Esc、键盘可达）；语义差异：语言选中即关面板，主题/外观保持打开便于连续试选。外观 `system` 档实时解析系统明暗并监听切换（`data-color-scheme` 仍只写二值，token 路由与插件帧广播不变）。文案全部经 `useT()` 三语字典（机制见 renderer-shell-routing 卡 i18n 段）。**瘦身（20260924-feature-sidemenu-settings-item）：身份头部（PopHead）与「设置」行内项已移出**——设置独立成底部导航项（见 SideMenu 段，`onOpenSettings` 改挂其上，与 `onOpenUser` 分离），`pop.user/pop.userHint/pop.settings` 键退役；**「收起/展开菜单」行内项保留**（带 `⌘/Ctrl+B` 快捷键提示，展开态收起动作的指针路径仍经面板）。面板定位 `left: calc(100% + 8px)` 贴入口右侧，二级 popover 锚定触发行右侧，延迟 180ms 关闭以允许指针移入；Esc 关闭；**Nav 不得设 `overflow: hidden`**（会裁剪面板、子菜单与 tooltip）。
