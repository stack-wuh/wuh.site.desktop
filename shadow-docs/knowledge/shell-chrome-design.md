...
domain: renderer-ui
keywords: [图标注册表, AppIcon, Icon, SideMenu, 菜单栏, 两栏布局, StatusBar, 状态栏, statusItems, 徽标, 插件贡献点, 主题 token, 品牌标, IconLogo, Dock 图标, 应用图标, ico, 任务栏, 浮窗, FloatLayer, float, main, toggle]
...
- 品牌/ Dock 图标几何改动必须同步 `brand.tsx` 与 `build/icon.svg`（含亮/暗变体 token 表），并重跑 `build:icon` 重新提交产物（png/ico/icns）；`tests/icon-build.test.ts` 校验同源几何、产物尺寸、ico 容器结构与重跑可复现。
...
- `vitest run tests/plugin-manifest.test.ts tests/plugin-statusitems.test.ts tests/plugin-assets.test.ts tests/icon-build.test.ts`（icon 用例覆盖设计源几何、PNG 尺寸、ico 容器结构与脚本重跑字节可复现；需 devDep `@resvg/resvg-js` 已安装）。
