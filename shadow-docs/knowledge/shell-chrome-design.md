...
verified-scope: components/SideMenu.tsx, components/menu/PluginTree.tsx, components/capsule/modules.tsx, lib/capsule.ts, components/capsule/CapsulePanel.tsx, src/plugin-sdk/index.ts, plugins/git-history
---
...
**SideMenu（2026-09-20 起，取代 ActivityBar）**：左栏菜单栏，两形态**瞬时切换**（禁 width 过渡，ui-patterns 布局位移动画禁令）——收起 = 48px 图标 rail，展开 = 220px 图标+文字（label 仅 opacity 淡入 200ms）。能力沿袭：左缘 2px 激活指示条（`::before`）、徽标位（数字 99+ 折叠 / `dot` 圆点，展开态转行内）、`data-tip` 自绘 tooltip（**仅收起态**显示，hover 与 `:focus-visible` 可见）+ `aria-label`、导航子树（展开态挂载 `TreeWrap`，行尾 TreeKnob 旋钮开合，`aria-label` 统一 `menu.treeToggle`）。结构：顶部**直接是导航项** → **底部系统区两项制**（`margin-top:auto` 吸附左下）：**【用户】在上（纯导航点击 /account）+【设置】在下（三态交互，20260925-feature-sidemenu-settings-consolidation）**——收起态图标 = 右箭头 `IconChevronRight`、点击仅展开菜单；展开态左 `IconSettings` 直达 `/settings` 且 `settingsActive` 高亮 `aria-current="page"`、右缘收起旋钮（TreeKnob 复用，stopPropagation 防误触导航）。**主导航【插件】单一可展开条目**（20260925 起，取代插件 main 视图平铺与独立 toggleItems 组）：`id=plugins`、新注册 `IconPuzzle`、常驻（子树空给「无启用插件」空态行）；子树 = `components/menu/PluginTree.tsx`——main 视图导航行（当前路由行高亮 `aria-current`）+ 浮窗开关行（`aria-pressed`、开着显示对勾）；收起态点条目仅展开菜单、展开态点条目切子树显隐（layout `pluginsTreeOpen`）。低频 chrome 控件不占导航黄金位。

**用户入口与快捷面板（20260925-feature-sidemenu-settings-consolidation 迁移化）**：底部两项制的用户项 = **纯导航**（点击直达 `/account`，`userActive` 命中时高亮走 `data-active` 稳定属性；悬停**不再弹任何面板**——外观与语言快捷面板已迁挂【设置】项，仅展开态弹出）。图标恒为品牌标 IconLogo（头像占位）——GitHub 头像投影（img）已回退：远程图片（avatars.githubusercontent.com）网络不可靠时常破损，**壳层禁止渲染远程头像图片，头像显示待 Settings「用户设置」（本地 name/avatar）接管**；展开态名字保留 GitHub 用户名文本投影（`name‖login`，未授权/stale 回落 wuh-site，文本走 API 响应无网络依赖，数据源见 renderer-shell-routing 卡「全局身份 store」段）；版本行随展开态显示。**快捷面板（原 UserQuickPanel，现挂设置项）**：`role="menu"` aria-label「外观与语言」= `pop.quickAria`——**主题（酒红/素雅）、外观（跟随系统/浅色/深色）、语言（中文/English/日本語）三组二级 popover 行**（「收起/展开菜单」行已退役：⌘/Ctrl+B 全局快捷键不变，指针路径 = 设置项右缘收起旋钮 / 收起态点击展开）——通用 `PopSubmenu` 组件：行左文案右 chevron、hover/聚焦弹出选项子菜单（`menuitemradio`、当前项勾选、180ms 延迟移入、Esc、键盘可达）；语义差异：语言选中即关面板，主题/外观保持打开便于连续试选。外观 `system` 档实时解析系统明暗并监听切换（`data-color-scheme` 仍只写二值，token 路由与插件帧广播不变）。文案全部经 `useT()` 三语字典（机制见 renderer-shell-routing 卡 i18n 段）。面板定位 `left: calc(100% + 8px)` 贴入口右侧，二级 popover 锚定触发行右侧，延迟 180ms 关闭以允许指针移入；Esc 关闭；**Nav 不得设 `overflow: hidden`**（会裁剪面板、子菜单与 tooltip）。
...
**壳层胶囊 Capsule 与任务中心（2026-09-24 起，事件总线 + 任务事件溯源 + 挂点入 header）**：壳层一等聚合入口，固定命名 `components/capsule/`——`Capsule.tsx`（chip，data-testid `capsule`）+ `CapsulePanel.tsx`（任务中心面板，`capsule-panel`）+ `sections/EditorSection.tsx`（编辑器分区，`EditorCommandHost` 常驻命令宿主由壳层 layout 单实例挂载）。**挂点契约（20260924-fix-capsule-header-chrome 迁移）：TitleBar（44px header）右侧 flex 子项**——chip 垂直居中、`margin-left:auto` 推右缘，面板贴 chip 下缘 10px **向下**弹出；层叠等价换算：TitleBar 不建层叠上下文，面板 z70 仍在根上下文（**>浮窗 z2、<Dialog 100**），chip 低于浮窗不变；Esc/点外关（window 级监听 + target 归属守卫，20260924-fix-capsule-self-close）不受影响；空态显示「就绪」，不整体隐藏。**chip 解剖（20260924 微进度环重设计，替代 20260923 状态环药丸）**：26px 药丸 **ghost 态**——header 上默认透明底无边框（安静的信令），hover/面板展开浮起 chrome-raised 药丸 + elevation-card 阴影；**微进度环（SVG 14px）**：环即聚合进度——空闲=空心轨道环、有任务=primary 进度弧（弧长=done/total）、有 in_progress 叠加旋转亮弧（800ms/圈，**属持续状态指示非过渡动效**，reduced-motion 静态降级）、全完成=success 满环；标签任务态整串 mono 语义色（进行中=primary、全完成=success）`任务 {done}/{total}`，空态「就绪」/编辑器入口 sans 次级色；chevron 开合旋转 180°（150ms，reduced-motion 关）。**面板 = 任务中心（420px 宽近满高，多插件并发任务的集散地）**：「任务 | 模块 | 插件 tab」动态 tab 结构（`role=tablist`；模块 tab = EditorSection + capsule 贡献点 tiles，20260923 控制中心现状保留）；**插件 tab（20260925-feature-capsule-plugin-tab 起，声明制贡献点）**：manifest 顶层 `tabs: [{id,title,icon}]`（每插件 ≤1，validateManifest 校验），运行时逻辑帧经 SDK `wuh.capsule.updateTab(id, { sections })` 上报结构化内容（sections[{title?,rows[{icon?,text,detail?,tone?,viewId?}]}]，护栏 ≤3 sections × ≤8 rows、序列化 ≤4KB、row.icon 白名单、row.viewId 须为本插件 main 视图——严格 typeof 校验在 `lib/capsule.ts` 注册表）、`removeTab` 隐藏；注册表 tab key = `plugin:<pid>:<tid>`（对齐 pluginPanelKey），声明注册默认 hidden（不出 tab 按钮）、非 git 目录「失败不打扰」静默隐藏；面板在「模块」后按声明序追加 tab 按钮（图标+manifest title），选中 tab 被移除回落 tasks，空 sections 显示「暂无数据」（`capsule.tabEmpty` 三语）；内容为通用 sections/rows 渲染器（分区间 border-top、行 = tone 色点/白名单图标 + mono 文本 + detail，含 viewId 整行可点跳 `/plugin/<id>/<view>` 并关面板）；tab 标题来自 manifest（不进壳层 i18n）；帧服务方法 `capsule.updateTab/removeTab` 走既有 capsule 帧服务（主进程 broker 不参与），三方同步 = shared/plugin.ts 类型 ↔ SDK 字符串（`src/plugin-sdk/index.ts`；protocol.ts 仅负责下发）↔ PluginFrameHost 裁决；任务 tab = **单流时间线**——左缘 2px 脊线（Rail）串状态标（pending 空心 / in_progress 旋转环 + 脊线段 primary 脉动 / done 对勾，圆形底色遮断脊线成珠串），排序 进行中（updatedAt 倒序）→ 待处理（createdAt 倒序）→「最近完成」（doneAt 倒序，置灰 65%）；行 = 状态标 + 标题 + 来源插件徽标（mono pill）+ 进度（细条 + n/m mono）+ detail + 相对时间（自算 s/m/h，禁 `toLocaleString`）；viewId 行整行可点跳来源视图。头部 = 聚合进度条（全完成转 success）+ mono 计数；筛选 chips（全部/未完成/已完成，`aria-pressed`）+ 插件 pill（≥2 插件有任务才出现）；空态分「等待插件任务」/「无匹配任务」；**「清空已完成」= 壳层唯一允许的写操作（remove 隐藏条目，不改状态）**；面板开着时新行 150ms 淡入（reduced-motion 关）。编辑器工具条 IconRow **横密纵疏**（column-gap 2px / row-gap 6px，20260924-fix-capsule-header-chrome——`gap` 横纵共用曾致换行工具条贴死）；模块卡基础件 `ModuleRow`（排版/快捷键手风琴行）纵向 margin **6px**（20260925-feature-sidemenu-settings-consolidation——曾 `margin: 0 4px` 致上下堆叠卡在子面板收起时贴死无间距）。
